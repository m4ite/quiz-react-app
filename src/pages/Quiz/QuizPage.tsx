...
function QuizPage() {
  ...
  const [respostaSelecionada, setRespostaSelecionada] = useState<string | null>(null); // índice da alternativa escolhida pelo usuário
  ...
  useEffect(() => {
    if (!categoria) { // se a categoria estiver vazia
      navigate('/'); // retorna para a página principal
      return;
    }

    const filtradas = arrayQuestoes.filter( // pegar só as questões da categoria escolhida
  ...
  if (questoes.length === 0) { // enquanto não tiver nada na lista e questões
  ...
  return (
    <div>
        ...
        {currQuestao.alternativas.map((option, idx) => (
          <li key={idx}>
            <button
              ...
              className={respostaSelecionada === option ? 'selected' : ''}
            >
              {option}
            </button>
          </li>
      ...
      <button onClick={proximaQuestao} disabled={!respostaSelecionada}>
        Next
      </button>
    </div>
  );
